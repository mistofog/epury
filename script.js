function f() {
    ...
    let y = document.getElementById("sechZ").value; // геометрическая характеристика сечения
    let z = document.getElementById("sechY").value; // геометрическая характеристика сечения
    let E = 70000; // модуль упругости
    console.log("sila " + Fp);
    console.log("shirina " + z);
    console.log("dlina " + y);
    ...
    if (F.value > 0 && F.value <= 20 && z > 0 && y > 0) {
        // проверка на допустимое значение силы: от 0 до 20
        ...
        let I = ((z * Math.pow(y, 3)) / 12).toFixed(3); // вычисляем значение момента инерции
        console.log("momemt I" + I);
        let sig = ((Fp * L * y) / (2 * I)).toFixed(3); // считаем нормальное распределение напряжений
        let f = ((Fp * Math.pow(L, 3)) / (3 * E * I)).toFixed(3); // считаем максимальное отклонение свободного конца балки
        let i = 0; // задаем нулевое значение для счетчика
        for (L; L >= 0; L -= 25) {
            // задаем цикл для расчета момента в разных точках балки для построения графика
        ...
        for (let x = 500; x >= 0; x -= 25) {
            // задаем цикл для расчета смещения  разных точек консоли для построения графика
            W[i] =
                ((Fp * Math.pow(L, 3)) / (3 * E * I)) *
                (1 - (3 * x) / (2 * L) + Math.pow(x, 3) / (2 * Math.pow(L, 3))); // считаем и записываем в массив отклонения в консоли
            i++; // увеличиваем значение счетчика на 1
        }
        ...
        moment_max.value = Math.round(M[0].toFixed(3)) + " Н·мм"; //обновляем значение максимального момента добавляя строку - "приписку" со значением "Н на мм"
        ...
        w_max.value = W[20].toFixed(3) + " мм"; //обновляем значение максимального смещения добавляя строку - "приписку" со значением "мм"
        ...
        img_1.onload = function () {
            ...
            var chart_1 = new Chart(ctx_1, {
                // Чтобы создать диаграмму, нам нужно создать экземпляр Chart класса и передать ему параметры, которые описаны ниже
                type: "line", // выбираем тип графика - линия
                data: {
                    // прописываем главные параметры графиков: координаты точек
                    labels: [-(z / 2), z / 2],
                    datasets: [
                        {
        ...
        // выводим сообщение об ошибке в случае не правильно введенном значении силы
        alert(
            "Что то пошло не так. Возможные причины ошибки:" + // значение ошибки
            "1) Вы ввели дробное значение силы через знак зяпятой, пожалуйста замените его на точку " + // значение ошибки
            "2) Вы ввели значение <0 или >20Н 3) Вы ввели отрицательное значение сечения или не ввели его вовсе"
        ); // значение ошибки
    }
}
